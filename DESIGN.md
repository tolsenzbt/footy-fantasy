...
- Each `nations` row stores `name`, `fifa_code` (canonical FIFA 3-letter, unique per nation), and `iso_code` (ISO 3166-1 alpha-2, unique — the authoritative key for emoji flag derivation; null for England/Scotland, which are GB subdivisions with no alpha-2 code and render via hardcoded tag-sequence emoji). All code joins on the internal `nations.id` UUID; the string codes are display-only.
...
- **`nations` codes canonicalized + `iso_code` added (June 9, migration 0013).** Audit during UI build found `fifa_code` held Wikipedia-squad conventions, not canonical FIFA codes — 19/48 nonstandard, including two collisions (AUS→Australia+Austria, IRA→Iran+Iraq). Fixed: added `iso_code` (ISO 3166-1 alpha-2, unique, drives emoji flags), corrected all 48 `fifa_code` to canonical FIFA codes (now unique, no collisions). Grep audit confirmed `fifa_code` is display-only — projection ingest resolves nations by name→UUID, every join is UUID-keyed — so canonicalization was display-safe. NationChip renders flag (from `iso_code`, with hardcoded tag-sequence emoji for ENG/SCO) + canonical `fifa_code`. The hand-maintained FIFA→ISO lookup (`nation-flags.ts`) is deleted.
- **`runGroupDraw` / `auto-lineups` undercount commissioners (found June 9).** Both filter `league_memberships.role = 'manager'` and assert the count equals league size. A commissioner is a league participant per §9 ("drafts a team, sets lineups"), so a league with a commissioner would undercount by one and fail the group draw / skip that manager's lineup. Not triggered for the inaugural league (no commissioner assigned), but must be fixed before the commissioner role is ever used — the role filter should include commissioners as participants.
- **`picks.ts` auto-fires `runGroupDraw` on the final pick (found June 9).** A fire-and-forget call runs the group draw the moment the last initial-draft pick lands. This contradicts §9/§16's design where `drafting→group_stage` is an explicit admin "run group draw and confirm" action (and the §4 admin-override / IRL-draw case requires NOT auto-drawing). Benign in script context (connection closes first, nothing written), but in production the real draft's final pick would trigger an unconfirmed group draw. Investigate + likely remove before the real draft; the explicit "Finalize Draft" action replaces it.
- **dev-seed auto-draft is nation-lopsided (found June 9).** Picks alphabetically within position, draining whole national squads (224 picks from 11 nations). Harmless for read-model validation but unrealistic; spread across nations when building the 8-team campaign seed.
