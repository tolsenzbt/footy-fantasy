...
- **Validation stats strategy (decided June 4):** The end-to-end campaign uses a hybrid data source, split by what each phase tests. Group stage: real 2022 WC match data replayed through the live ingestion sweep — exercises concededWhileOnPitch derivation and the §6 engine against realistic API-Football-shaped payloads (the just-audited, highest-risk code). Knockouts: synthetically constructed real_fixtures + stats matching the 2026 fantasy structure exactly (a proper 16-match real R32 → fantasy qf, etc.). Rationale: 2022 had no R32 and the wrong fixture counts; for the bracket/redraft/resolver code, structurally-correct synthetic data is better validation than real-but-wrong-shape replay. The hand-built knockout fixtures also exercise mapRound against best-guess 2026 round strings end-to-end (does NOT substitute for the live /fixtures/rounds verification before June 28). Campaign runs against a fresh isolated league; nothing deleted.
- **Group-stage validation COMPLETE (June 5–6).** The full group stage (MD1, MD2, MD3) was validated end-to-end against the deployed DB using real 2022 WC stat data replayed through the live ingestion sweep via SweepDeps injection (per-fixture remap onto 2026 nations, Option A correctness mapping). Validated: concededWhileOnPitch derivation incl. own-goal attribution (855767), the §6 scoring engine, player_match_stats/scores upserts, resolveMatchups, multi-matchday standings accumulation (PF/W/L/D summing across 3 matchdays), the §2 tiebreaker chain (head-to-head rung fired and resolved correctly on a real PF tie), advancement (top-2-per-group → 8 managers, QF bracket seeded), idempotency, and the MD3 mass-release waiver-event scheduling. Validation surfaced and fixed THREE production bugs that would have hit the live league: (1) upsertPlayerMatchStats crashed on null penalty/card fields the API returns despite the type claiming non-null — fixed at the derivation source in conceded.ts (corrected ApiPlayerStatistics nullability + coercion); (2) resolveBracket guarded ALL seed resolution behind a null-check, so QF seeds locked at first resolution and never updated as standings changed across matchdays — fixed so standing-type seeds always re-resolve while winner-type seeds stay final-once-set; (3) mapRound R16→qf miswiring (fixed earlier). Nation elimination at group_md3 is correctly a no-op until knockout fixtures exist (absence-based logic) — deferred to the knockout phase. NOT yet validated: the knockout half (synthetic fixtures), redraft, mass-release execution, priority reset, bracket advancement through to champion — these are the next campaign phases.
- **Knockout-transition validation IN PROGRESS (phase 4).** Synthetic knockout fixtures (2022 had no R32; Option C synthetic approach). Phase 4a validated: synthetic qf real_fixtures build, absence-based nation elimination (fires once knockout fixtures exist — correctly a no-op before then), §8 mass-release (drops eliminated-nation players from advancing managers only; eliminated managers' rosters lock intact; per-manager drop count is the §7 by-need redraft input). Phase 4a surfaced bug #4: **leagueMemberships.eliminatedAtRound was never written by any production code** — the resolve chain set nations.eliminatedAtRound but never marked non-advancing MANAGERS eliminated, so runMassRelease (which reads isNull(eliminatedAtRound) to identify advancing managers) would have treated all managers as advancing and over-dropped the eliminated managers' locked rosters (silent corruption, no crash). Fixed: setManagerEliminations (src/lib/standings/manager-elimination.ts) called from resolveRound after computeStandings for group_md3, format-aware cutoff (8-team top-3 advance, 12/16 top-2). Remaining phase 4: redraft (4b), knockout ingestion + bracket advancement to champion (4c).
